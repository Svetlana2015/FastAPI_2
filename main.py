...
# Transform variables, merge with existing df and keep column names
numeric[numeric.columns] = pd.DataFrame(scaler.transform(numeric), index= numeric.index)
#numeric=pd.concat([numeric_2, numeric.TARGET], axis=1)
df = pd.concat([numeric,encoded_variables], axis=1) 
...
def predict(datas: Parametres):
    ...
    numeric=df.select_dtypes(exclude=['object'])
    numeric[numeric.columns] = pd.DataFrame(scaler.transform(numeric), index= numeric.index)
    df = pd.concat([numeric,encoded_features], axis=1) 
    print('done')
